...
class Prompter(object):
    ...
    def __init__(self, template_name: str = "/content/drive/MyDrive/Data/chinese/alpaca.json", verbose: bool = False):
        self._verbose = verbose
        '''if not template_name:
            # Enforce the default here, so the constructor can be called with '' and will not break.
            template_name = "alpaca"
        file_name = os.join("templates", f"{template_name}.json")
        if not os.exists(file_name):
            raise ValueError(f"Can't read {file_name}")'''
        file_name = '/data/2t2/zty/hyz/Llama-Chinese/data/alpaca.json'
...
def train():
    ...
    if training_args.do_eval:
        ...
        metric = evaluate.load("/data/2t2/zty/hyz/Llama-Chinese/accuracy.py")
